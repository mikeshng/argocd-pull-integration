...
# OCM bundle version - must be set via environment variable (default defined in Makefile)
OCM_BUNDLE_VERSION=${OCM_BUNDLE_VERSION:?OCM_BUNDLE_VERSION must be set}
...
echo "############ Init hub (OCM bundle version: ${OCM_BUNDLE_VERSION})"
...
$(echo ${joincmd} --force-internal-endpoint-lookup --wait --bundle-version=${OCM_BUNDLE_VERSION} | sed "s/<cluster_name>/cluster1/g")
